...
/*
    ...
    Description:
        Registers a task as a attack type of task.
...
[QGVARMAIN(initFramework), {
    ...
    if (_limitSuccess > count GVAR(allTargets)) then {
        [COMPONENT_STR, "WARNING", format [
            "Required number of targets to attack (set to %1) is higher than the target count (current: %2)", _limitSuccess, count GVAR(allTargets)
